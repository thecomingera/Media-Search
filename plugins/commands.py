...
async def start(bot, message):
    ...
    if len(message.command) > 1 and message.command[1] == 'subscribe':
        ...
        buttons = [
            [
                InlineKeyboardButton('Updates Channel 🗣', url='https://t.me/piratedcitypc'),
...
async def total(bot, message):
    ...
    msg = await message.reply("Processing...⏳\n@StreamerView Project", quote=True)
    try:
        total = await Media.count_documents()
        await msg.edit(f'📁 Total files saved in @StreamerView Media Search Bots Database: {total}')
